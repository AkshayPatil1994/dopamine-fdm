...
Contains
  ...
  !> Compute RHS for scalar advection-diffusion (MUSCL advection + central diffusion)
    ...
    Do k = 2, nzg-1
       Do j = 2, nyg-1
          Do i = 2, nxg-1
             ...
             dy_f  = y(j) - y(j-1)
             adv_y = ( (V_(i,j,k) - ws)*C_hi - (V_(i,j-1,k) - ws)*C_lo ) / dy_f
             ...
             kappa_hi = nu/Sc + 0.5d0*(nu_t(i,j,k) + nu_t(i+1,j,k))/Sc_t
             kappa_lo = nu/Sc + 0.5d0*(nu_t(i,j,k) + nu_t(i-1,j,k))/Sc_t
             diff_x = ( kappa_hi*(C_(i+1,j,k) - C_(i,j,k))/(xg(i+1)-xg(i)) &
                      - kappa_lo*(C_(i,j,k) - C_(i-1,j,k))/(xg(i)-xg(i-1)) ) / dx_f

             !-------- y-diffusion -----------------------------------------
             kappa_hi = nu/Sc + 0.5d0*(nu_t(i,j,k) + nu_t(i,j+1,k))/Sc_t
             kappa_lo = nu/Sc + 0.5d0*(nu_t(i,j,k) + nu_t(i,j-1,k))/Sc_t
             diff_y = ( kappa_hi*(C_(i,j+1,k) - C_(i,j,k))/(yg(j+1)-yg(j)) &
                      - kappa_lo*(C_(i,j,k) - C_(i,j-1,k))/(yg(j)-yg(j-1)) ) / dy_f

             !-------- z-diffusion -----------------------------------------
             kappa_hi = nu/Sc + 0.5d0*(nu_t(i,j,k) + nu_t(i,j,k+1))/Sc_t
             kappa_lo = nu/Sc + 0.5d0*(nu_t(i,j,k) + nu_t(i,j,k-1))/Sc_t
  ...
  Subroutine update_ghost_scalar(C_)
    ...
    Call Mpi_Sendrecv( sc_snd_lo, nxg*nyg, Mpi_real8, sendto,   tagto,   &
                       sc_rcv_lo, nxg*nyg, Mpi_real8, recvfrom, tagfrom, &
                       MPI_COMM_WORLD, istat, ierr )
    If (myid /= 0) C_(:,:,1) = sc_rcv_lo(:,:,1)
    ...
    sc_snd_hi(:,:,1) = C_(:,:,2)
    Call Mpi_Sendrecv( sc_snd_hi, nxg*nyg, Mpi_real8, sendto,   tagto,   &
                       sc_rcv_hi, nxg*nyg, Mpi_real8, recvfrom, tagfrom, &
                       MPI_COMM_WORLD, istat, ierr )
    If (myid /= nprocs-1) C_(:,:,nzg) = sc_rcv_hi(:,:,1)

  End Subroutine update_ghost_scalar


  !> Apply boundary conditions to scalar C (x-periodic, z-MPI halo, y-wall fluxes)
  Subroutine apply_scalar_bc(C_)

    Real(Int64), Dimension(nxg, nyg, nzg), Intent(InOut) :: C_

    ! x-periodicity (cell centres)
    C_(1,   :,:) = C_(nxg-2,:,:)
    C_(nxg-1,:,:) = C_(2,   :,:)
    C_(nxg,  :,:) = C_(3,   :,:)
